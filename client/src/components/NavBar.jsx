...
const NavBar = () => {
    ...
    return (
        <div className='flex items-center justify-around flex-col sm:flex-row gap-2 my-2'>
            ...
            onClick={() => { navigate('/') }}>Brain Wars</h1>
            <button className='sm:hidden' onClick={()=>setOpen(!open)}>
                {!open ? '☰' : 'X'}</button>
            {
            <div className={`sm:flex flex-col sm:flex-row justify-evenly sm:min-w-md text-center gap-2 ${open ? 'flex': 'hidden'}`}>
                <a href="https://www.github.com/abhinav-dev0/brain-wars" target='_blank'>Source Code</a>
                <a href="https://www.linkedin.com/in/abhinav-dev0" target='_blank'>Connect with Me</a>
            </div>
            }
        </div>
    )
